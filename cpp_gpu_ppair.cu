...
double optimize(unsigned max_steps, double *x, double *y, double *z,
  ...
  for (unsigned step = 0; step < max_steps; ++step) {
    /*score = */evaluate<<<num_blocks, block_size>>>(x, y, z, ppairs, scores, n_ppairs);
...
int main() {
  ...
  unsigned n_ppairs = n_particles * (n_particles - 1) / 2;
  ...
  optimize(1000, x, y, z, ppairs, scores, n_ppairs);
